...
/**
 * The class for the obsidian boat item
 */
public class ItemObsidianBoat extends Item {
	public ItemObsidianBoat() {
		super();
		this.setUnlocalizedName("obsidianBoat");
		this.setTextureName(CppReferences.MODID + ":" + "obsidian_boat");
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectType.BLOCK)
                {
                    ...
                    EntityObsidianBoat obsidianboat = new EntityObsidianBoat(world, (double)((float)i + 0.5F), (double)((float)j + 1.0F), (double)((float)k + 0.5F));
                    obsidianboat.rotationYaw = (float)(((MathHelper.floor_double((double)(player.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!world.getCollidingBoundingBoxes(obsidianboat, obsidianboat.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return itemstack;
                    }
